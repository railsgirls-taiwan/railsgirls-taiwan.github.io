...
**確認電腦已安裝 Rails**，尚未安裝請參考[**安裝指南**](/install)。
...
{% endhighlight %}
    ...
    <div>
可以用 <code>ls</code> 命令來檢查叫做 <code>projects</code> 的資料夾有沒有產生。應該會在輸出裡看到 <code>projects</code> 資料夾。現在你想從當下資料夾進入 <code>projects</code> 資料夾，可以執行：
    </div>
...
{% endhighlight %}
    ...
    <div>
可以用 <code>dir</code> 命令來檢查叫做 <code>projects</code> 的資料夾有沒有產生。應該會在輸出裡看到 <code>projects</code> 資料夾。現在你想從目前的資料夾進入 <code>projects</code> 資料夾，可以執行：
    </div>
...
鷹架在專案資料夾下建立了一些新檔案，要讓這些檔案生效，需要執行一些指令，來更新資料庫、並重新啟動伺服器。
...
修改過後記得存檔，並重新整理瀏覽器，來看看什麼被改變了。妳也可以繼續調整 HTML 與 CSS。
